...
// Round meters to exact intervals: 12000, 6000, 3000, 1500, 750, 250, 0
function formatMeters(meters: number): string {
  // Find the closest number in the allowed set.
  const intervals = [12000, 6000, 3000, 1500, 750, 250, 0];
  let closest = intervals[0];
  let minDiff = Math.abs(meters - closest);

  for (let i = 1; i < intervals.length; i++) {
    const diff = Math.abs(meters - intervals[i]);
    if (diff < minDiff) {
      minDiff = diff;
      closest = intervals[i];
    }
          ...
          <div className="rounded-2xl border border-white/8 bg-black/60 p-4 shadow-2xl shadow-black/40 backdrop-blur-2xl">
                  ...
                  {showLogo ? (
                    <span className="relative flex h-18 w-18 items-center justify-center overflow-hidden rounded-xl border border-black/10 bg-white/95 p-3.5 shadow-sm">
                      ...
                      <Image
                        ...
                        width={68}
                        height={68}
                        className={`relative h-13 w-13 object-contain transition-opacity duration-200 ${logoLoaded ? "opacity-100" : "opacity-0"
                          }`}
                        unoptimized
                        onLoad={() => {
              ...
              <div className="flex items-center gap-1.5">
                {onToggleFpv && (
                  <motion.button
                    ...
                    className={`flex h-6 w-6 items-center justify-center rounded-full transition-colors ${isFpvActive
                ...
                value={
                  settings.altitudeUnit === "meters"
                    ? flight.baroAltitude !== null
                      ? `${formatMeters(flight.baroAltitude)} m`
                      : "—"
            ...
            <div className="mt-2.5 flex flex-col gap-1.5">
              ...
              {flight.squawk && (
                <div className="flex items-center gap-1.5">
                  <span
                    className={`h-3 w-3 text-center text-[8px] font-bold leading-3 ${isEmergencySquawk(flight.squawk)
                      ? "text-red-400"
                      : "text-white/25"
                      }`}
                  >
                  ...
                  <p
                    className={`font-mono text-[11px] font-medium tracking-wide ${isEmergencySquawk(flight.squawk)
                      ? "text-red-400"
                      : "text-white/40"
                      }`}
                  >
