...
function FlightTrackerInner() {
  ...
  const handleNorthUp = useCallback(() => {
    window.dispatchEvent(new CustomEvent("skypulse:north-up"));
  }, []);

  const handleResetView = useCallback(() => {
    window.dispatchEvent(
      new CustomEvent("skypulse:reset-view", {
  ...
  const handleOpenSearch = useCallback(() => {
    window.dispatchEvent(new CustomEvent("skypulse:open-search"));
  }, []);
  ...
  return (
    <main className="relative h-dvh w-screen overflow-hidden bg-black">
      ...
      <div
        ...
        style={{ top: "calc(max(env(safe-area-inset-top, 0px), 36px) + var(--ad-banner-height, 0px))" }}
      >
        {!fpvIcao24 && (
