...
export function CameraController({
  ...
  useEffect(() => {
    ...
    window.addEventListener("skypulse:north-up", onNorthUp);
    window.addEventListener("skypulse:reset-view", onResetView);

    return () => {
      window.removeEventListener("skypulse:north-up", onNorthUp);
      window.removeEventListener("skypulse:reset-view", onResetView);
    };
  }, [map, isLoaded, city]);
  ...
  useEffect(() => {
    ...
    window.addEventListener("skypulse:camera-start", onStart);
    window.addEventListener("skypulse:camera-stop", onStop);

    return () => {
      window.removeEventListener("skypulse:camera-start", onStart);
      window.removeEventListener("skypulse:camera-stop", onStop);
      if (frameId != null) cancelAnimationFrame(frameId);
    };
  }, [map, isLoaded]);
  ...
  useEffect(() => {
    ...
    window.addEventListener("skypulse:camera-stop", onCameraStop);
    ...
    return () => {
      ...
      window.removeEventListener("skypulse:camera-stop", onCameraStop);
    };
  }, [
    map,
    isLoaded,
